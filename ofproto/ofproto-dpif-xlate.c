...
static int
tnl_route_lookup_flow(struct xlate_ctx *ctx,
                      ...
                      struct xport **out_port)
{
    ...
    group = ovs_router_lookup_group(oflow->pkt_mark, &dst, src);
    if (group && ovs_router_group_is_multipath(group)) {
        if (oflow->dp_hash) {
            ctx->wc->masks.dp_hash |= ovs_router_group_hash_mask(group);
        } else if (ctx->xbridge->support.odp.recirc) {
            enum ovs_hash_alg hash_alg = OVS_HASH_ALG_SYM_L4;

            if (hash_alg > ctx->xbridge->support.max_hash_alg) {
                /* Algorithm supported by all datapaths. */
                hash_alg = OVS_HASH_ALG_L4;
            }
            ctx_trigger_recirculate_with_hash(ctx, hash_alg, 0);
            xlate_report(ctx, OFT_DETAIL,
                         "native tunnel ECMP using dp_hash, recirculating");
            ovs_router_group_unref(group);
            return -EAGAIN;
        } else {
            flow_mask_hash_fields(oflow, ctx->wc,
                                  NX_HASH_FIELDS_SYMMETRIC_L3L4_UDP);
        }
    }

    /* Reuse a hash generated by an earlier datapath action.  The direct
     * inner-flow hash lets datapaths without recirculation retain ECMP. */
    hash = oflow->dp_hash
           ? oflow->dp_hash
           : flow_hash_fields(oflow, NX_HASH_FIELDS_SYMMETRIC_L3L4_UDP, 0);
    if (group) {
        bool found = ovs_router_group_select(group, src, hash, &result);

        ovs_router_group_unref(group);
        if (!found) {
            return -ENOENT;
        }
    } else if (!ovs_router_lookup_with_hash(oflow->pkt_mark, &dst, src, hash,
                                            &result)) {
        return -ENOENT;
    }
    ovs_strlcpy(out_dev, result.output_netdev, sizeof out_dev);
    *src = result.src;

    if (ipv6_addr_is_set(&result.gw) &&
        (!IN6_IS_ADDR_V4MAPPED(&result.gw) ||
         in6_addr_get_mapped_ipv4(&result.gw))) {
        *ip = result.gw;
    } else {
        *ip = dst;
    }
